	...
	<script type="text/javascript">
		...
		function chargeSwitch() {
			...
			if (flag) {
				...
				Pile.pileStart(CONFIGS.TURLManage().startChargeApi, userid, deviceId, function(res) {
				...
				mui.confirm("是否结束本次充电", "您点击了停止充电", ["否", "是"], function(e) {
					...
					var user = User.userIsLogin();
					var userid = user.userId;
					Pile.pileStop(CONFIGS.TURLManage().stopChargeApi, userid, function(res) {
						console.log(res);
						$("#mask").addClass("hidden");
						$("#chargeSwitch").removeClass("disabled");
						if (res) {
							...
							//获取流水号
							Pile.getSerialNo(CONFIGS.TURLManage().getSerialNoApi, user.userId, function(res) {
								if (res) {
		...
		function getChargeData() {
			User.getPileChargeData(CONFIGS.TURLManage().getRealTimeData, user.userId, function(DataInfo) {
				if (DataInfo) {
		...
		//强制退出事件
		function userChange() {
			User.UserChange(CONFIGS.TURLManage().changeUserStateApi, userid, function(e) {
				if (e) {
					WeixinJSBridge.call('closeWindow');
				}
			});

(function(config) {
	...
	function rc() {
		for(var a = mc + "/maps/main?v=" + config[4] + "&key=" + config[0] + "&m=" + config.AN.join(",") + "&vrs=1503546983737", b = document.getElementsByTagName("script"), c, d = 0; d < b.length; d += 1)
			if(0 === b[d].src.indexOf(mc.split(":")[1] + "/maps?")) {
				c = b[d];
				break
			}
...
//		root_s.parentNode.insertBefore(cnzz_s_tag, root_s);
	...
	try {
		if(window.localStorage) {
			...
			if(uc)
				for(Xb in window.localStorage) window.localStorage.hasOwnProperty(Xb) && 0 === Xb.indexOf("_AMap_") && window.localStorage.removeItem(Xb)
		}
	} catch(vc) {}
